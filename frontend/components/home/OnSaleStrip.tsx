...
export default function OnSaleStrip() {
  const c = useCarousel();
  const { data: sale = [], isLoading } = useQuery<Product[]>({
    queryKey: ["home-sale", { limit: 8 }],
    queryFn: () => fetchOnSale(8),
  });
  const multi = (sale?.length || 0) > 1;
        ...
        >
          <div>
            <h2 className="mb-2 font-serif text-ds-section text-charcoal-ink">
              On Sale{" "}
              <em className="italic-accent text-fountain-navy">Now</em>
            </h2>
